...
xSkel::readSkelV1(FILE * file)
 ...
 void 
 xSkel::saveFile(std::string fname)
 {

           
   char skelid[] = "SKELBINF002"; //12bytes
   char skinid[] = "SKINPAR";   //8bytes
   int reserve[] = {0,0,0,0}; //16 bytes
   FILE * file;
   int i;
   xBone * a;
   ...
   fwrite(skelid, 1, 12, file);
   fwrite(&numBone, 1, 4, file);
   fwrite(&numVert, 1, 4, file);
   fwrite(reserve, 4, 4, file); //16 bytes reserved
  ...
  //SKIN POSE
    ...
    {
      a = &vecBone[i];
      fwrite(&a->id, 4, 1, file);
      fwrite(&a->parent, 4, 1, file);    
      fwrite(&a->depth, 4, 1, file);
      fwrite(&a->pos, 12, 1, file);
      fwrite(&a->ori, 16, 1, file);  
      fwrite(&a->scale,12, 1, file); //scale in this format is always  3float (1,1,1)      
    }//nexti

  //VERTEX WEIGHTS
    fwrite(skinid, 1, 8, file); //8 bytes "SKINPAR"+0

    fwrite(vecVert, numVert, sizeof(xSkVert), file);
...
void 
xSkel::reIndexSkin(short * vec, int num)
{
  if (numVert != num) { printf("reIndexSkin  error -- number of vertices is different orig:[%d] p:[%d] \n", numVert, num); }

  xSkVert * temp;
  temp = new xSkVert[numVert];
  ...
  //replace vertex array with new one
    delete [] vecVert;
    vecVert = temp;
